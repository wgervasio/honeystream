...
//
// The background script provides monitoring of tabs for an active Honeystream
...
// Observed tabs on Honeystream URL
const watchedTabs = new Set()

// Store for active tabs state
const tabStore = {}

// Used to know which honeystream instance to sent browser badge requests to
let lastActiveTabId
...
const TOP_FRAME = 0
const HEADER_PREFIX = 'x-honeystream'
const HONEYSTREAM_APP_URL = 'https://app.gethoneystream.com'
const isHoneystreamUrl = url =>
  url.startsWith(HONEYSTREAM_APP_URL) ||
  url.startsWith('http://local.gethoneystream.com') ||
...
const sendWebviewEventToHost = async (tabId, frameId, message) => {
  ...
  sendToHost(
    tabId,
    { type: 'honeystream-webview-event', payload: message, framePath },
    { frameId: TOP_FRAME }
  )
}

const getLastActiveHoneystreamTabId = () => {
  const isHoneystreamOpen = watchedTabs.size > 0
  if (!isHoneystreamOpen) return -1
...
// Add Honeystream header overwrites
...
// Allow embedding any website in Honeystream iframe
const onHeadersReceived = details => {
  const { tabId, frameId, responseHeaders: headers } = details
  let permitted = false

  // Whether these headers are within the context of a frame embedded in app.gethoneystream.com
  const isHoneystreamEmbedFrame = watchedTabs.has(tabId) && !isTopFrame(details)

  const isServiceWorkerRequest = watchedTabs.size > 0 && tabId === -1 && frameId === -1
  const shouldModify = isHoneystreamEmbedFrame || isServiceWorkerRequest
    ...
    for (let i = headers.length - 1; i >= 0; --i) {
      ...
      switch (header) {
        ...
        case 'set-cookie': {
          ...
          // Allow third-party cookies specifically in Honeystream tabs
...
// Programmatically inject content scripts into Honeystream subframes
const initScripts = details => {
  ...
  if (isTopFrame(details) && !popupTabs.has(tabId)) {
    // Listen for top frame navigating away from Honeystream
    if (!isHoneystreamUrl(details.url)) {
      stopWatchingTab(tabId)
    }
  } else {
...
const initializeWebview = details => {
  ...
  sendToHost(hostId, { type: `honeystream-webview-init${webviewId}`, payload: { tabId, frameId } })
...
//=============================================================================
// Honeystream tab management
//=============================================================================

const startWatchingTab = tab => {
  const { id: tabId } = tab
  console.log(`Honeystream watching tabId=${tabId}`)
  watchedTabs.add(tabId)
...
const stopWatchingTab = tabId => {
  ...
  console.log(`Honeystream stopped watching tabId=${tabId}`)
}


const PERMISSION_ACTIONS = {
  allowPopups(request) {
    // Allow Honeystream to open two popups at the same time without one getting blocked.
...
// Fetch on behalf of Honeystream app, skips cross-domain security restrictions
const request = async (tabId, requestId, url, options) => {
  ...
  const action = {
    type: `honeystream-fetch-response${requestId}`,
    payload: {
      err,
...
function messageHandler(action, sender, sendResponse) {
  ...
  // Listen for Honeystream app initialization signal
  if (action.type === 'honeystream-init' && isHoneystreamUrl(sender.tab.url)) {
    startWatchingTab(sender.tab)
    sendResponse(true)
    return
  }

  // Filter out messages from non-Honeystream app tabs
  if (!watchedTabs.has(tabId)) return

  switch (action.type) {
    case 'honeystream-webview-event':
      handleWebviewEvent(sender, action)
      break
    case 'honeystream-fetch': {
      const { requestId, url, options } = action.payload
      request(tabId, requestId, url, options)
      break
    }
    case 'honeystream-popup-init': {
      const { id: webviewId } = action.payload
      popupParents[webviewId] = tabId
      break
    }
    case 'honeystream-focus': {
      const targetTabId = getLastActiveHoneystreamTabId()
      if (targetTabId > -1) {
    ...
    case 'honeystream-permissions-request': {
...
// pause media in all non-honeystream tabs
...
const openLinkInHoneystream = details => {
  const { url: requestUrl, currentTime, source } = details

  const { protocol } = new URL(requestUrl)
  if (protocol !== 'http:' && protocol !== 'https:') return

  console.log(`Opening URL in Honeystream: ${requestUrl}${currentTime ? ` @ ${currentTime}` : ''}`)

  const targetTabId = getLastActiveHoneystreamTabId()
  if (targetTabId > -1) {
    sendToHost(targetTabId, {
      type: 'honeystream-extension-request',
    ...
    const url = `${HONEYSTREAM_APP_URL}/?${params.toString()}`
...
const openTabInHoneystream = async ({ tab, source }) => {
  const { id: tabId, url } = tab
  if (tabId < 0) return

  // ignore badge presses from Honeystream tabs
  if (watchedTabs.has(tabId)) return

  const currentTime = await getMediaTimeInTab(tabId)
  openLinkInHoneystream({ url, currentTime, source })
}

chrome.browserAction.onClicked.addListener(tab =>
  openTabInHoneystream({ tab, source: 'browser-action' })
)

//=============================================================================
// Create context menu items to add links to honeystream session
//=============================================================================

const TARGET_URL_PATTERNS = ['https://*/*']

chrome.contextMenus.create({
  title: 'Open link in Honeystream session',
  contexts: ['link'],
  targetUrlPatterns: TARGET_URL_PATTERNS,
  onclick(info, tab) {
    const { linkUrl: url } = info
    if (url) openLinkInHoneystream({ url, source: 'context-menu-link' })
  }
})
...
chrome.contextMenus.create({
  ...
  onclick(info, tab) {
    openTabInHoneystream({ tab, source: 'context-menu-browser-action' })
  }
})

chrome.contextMenus.create({
  title: 'Open video in Honeystream session',
  contexts: ['video'],
  targetUrlPatterns: TARGET_URL_PATTERNS,
  onclick(info, tab) {
    const { srcUrl: url } = info
    if (url) openLinkInHoneystream({ url, source: 'context-menu-video' })
  }
})
