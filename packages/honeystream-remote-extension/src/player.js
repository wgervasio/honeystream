...
;(function() {
  console.debug(`Honeystream player content script ${location.href}`)
  ...
  chrome.runtime.onMessage.addListener(action => {
    if (typeof action !== 'object' || typeof action.type !== 'string') return
    if (action.type === 'honeystream-host-event') {
  ...
  const mainWorldScript = function() {
    ...
    // Injected by Honeystream
    debug(`Honeystream main world script ${location.href}`)

    // Hide opener when Honeystream opens media in a popup.
    // Prevents pages from redirecting app.gethoneystream.com, but still allows
    // for app.gethoneystream.com to control the popup.
    let opener
    if (window.opener) {
    ...
    // Dispatch event
    // main world -> content script world -> background -> honeystream content script -> honeystream app
    ...
    const eventMiddleware = event => {
      ...
      debug(`[Honeystream Remote] Received player event`, action)
    ...
    window.MediaMetadata = class HoneystreamMediaMetadata extends MediaMetadata {
    ...
    class MediaSessionProxy {
      ...
      validateHandlers() {
        ...
        // Honeystream needs to be able to set the playback state accurately.
    ...
    class HTMLMediaPlayer {
      ...
      play() {
        if (this.dispatch('honeystreamplay')) return
      ...
      pause() {
        this.stopWaitingListener()
        if (this.dispatch('honeystreampause')) return
      ...
      seek(time) {
        if (this.dispatch('honeystreamseek', time)) return
      ...
      onWaiting(event) {
        ...
        const tryPlayback = () => {
          ...
          if (dt > 1) {
            ...
            this.dispatch('honeystreampause') || this.media.pause()
            const playPromise = this.dispatch('honeystreamplay') || this.media.play()
...
${ignoredSelectors}:empty {
  visibility: hidden !important;
    ...
    // TODO: This comparison logic should happen in the honeystream app where we
    ...
    const mediaElements = window.__honeystreamMediaElements
    if (mediaElements) {
      Array.from(mediaElements).forEach(addMedia)
      window.__honeystreamMediaElements = undefined
    }
  }
