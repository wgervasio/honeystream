...
;(function() {
  // Only run in iframes, the same as Honeystream webviews
  if (window.self === window.top) return

  const mainWorldScript = function() {
    document.getElementById('honeystreaminitscript').remove()
    ...
    window.__honeystreamMediaElements = new Set()

    // Proxy document.createElement to trap media elements created in-memory
    const origCreateElement = document.createElement
    const proxyCreateElement = function() {
      const element = origCreateElement.apply(document, arguments)
      if (window.__honeystreamMediaElements && element instanceof HTMLMediaElement) {
        window.__honeystreamMediaElements.add(element)
      }
      return element
    }
    ...
    setTimeout(() => {
      if (window.__honeystreamMediaElements) {
        window.__honeystreamMediaElements.clear()
    ...
    if (isFirefox) {
      ...
      if (!navigator.mediaSession) {
        ...
        const mediaSessionStub = {
          __installedByHoneystreamRemote__: true,
  ...
  script.id = 'honeystreaminitscript'
