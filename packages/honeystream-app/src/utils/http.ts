  ...
  if (typeof headers === 'object') {
    ...
    Object.keys(headers).forEach(name => {
      if (transformList.has(name.toLowerCase())) {
        headers[`X-Honeystream-${name}`] = headers[name]
      }
    })
  }
  return headers
...
const mainFetch = (url: string, options: FetchOptions = {}): Promise<FetchResponse> => {
  return new Promise((resolve, reject) => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (data.type !== `honeystream-fetch-response${requestId}`) return
    ...
    dispatchExtensionMessage('honeystream-fetch', { requestId, url, options })
  })
}
