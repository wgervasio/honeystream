...
import createClient, { SignalClient } from '@honeystream/signal-server/lib/client'
import { SignalErrorCode } from '@honeystream/signal-server/lib/types'
...
export class WebRTCPeerCoordinator extends PeerCoordinator {
  ...
  private getClient() {
    return createClient({
      server: HONEYSTREAM_SIGNAL_SERVER,
      connectTimeout: NETWORK_TIMEOUT,
      peerOpts: {
        config: {
          iceServers: HONEYSTREAM_ICE_SERVERS
        }
      }
    })
  }
