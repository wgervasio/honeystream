...
Keep the existing typed simulated transport lab as the source of performance proof, add explicit coverage assertions for the YouTube, AnimePahe, Cineby, Miruro, and generic streaming fixture matrix, and surface the home-page connection lab highlights for latency budget, selected-lane byte loss, and site coverage.

## Consequences

The branch now ties the cozy home UX to the deterministic mock-connection gate while preserving the host-authoritative 1:1 architecture. Future runtime adapters should keep media bytes local, preserve zero-loss selected lanes, and update the visible lab proof if fixture count or latency budgets change.
